...
def send_ether(addr_from, addr_to, amount):
	...
	if not isinstance(addr_from, str):
		addr_from = hex(addr_from)[2:]

	if not isinstance(addr_to, str):
		addr_to = hex(addr_to)[2:]
...
def get_storage_value( address, index, st_blocknumber, read_from_blockchain = False ):
	...
	if read_from_blockchain:
		if index in MyGlobals.initial_storage:
			value=MyGlobals.initial_storage[index]
		else:
			value = 0
		return value
	else:
...
def binary( o1, o2 , step, op='NONE'):
	if op == 'SHL':return (o2*2**o1) % (2 ** 256)
	if op == 'SHR':return floor(o2/2**o1)
	if op == 'SAR':return floor(o2/2**o1)
	if op== 'SIGNEXTEND':
		o = o2
		t = 256 - 8 * (o1 + 1)
		tbit = (o >> t) & 1
...
def execute( code, stack, pos, storage, temp_storage, mmemory, data, st_blocknumber, debug=False, read_from_blockchain = False ):
	...
	if op=='SLOAD' or op == 'SSTORE':
		MyGlobals.stack_record.append((step,op, stack[-1]))
	if op=='JUMP' or op == 'JUMPI':
		MyGlobals.stack_record.append((step,op, stack[-1]))
	if op == 'AND':
		str0=str(hex(stack[-1]))[2:]
		str1=str(hex(stack[-2]))[2:]

		if str0.count('f')>0 and str0.count('0')+ str0.count('f') ==len(str0):
			MyGlobals.stack_record.append((step,op, stack[-1]))
		elif str1.count('f')>0 and str1.count('0')+ str1.count('f') ==len(str1):
			MyGlobals.stack_record.append((step,op, stack[-2]))
		else:
			print('cannot handle this AND')
	...
	if op == 'SLOAD':
		...
		if addr == undefined:
			...
			return stack, pos, True, mmemory
		MyGlobals.concrete_read.add(addr)
		if addr in storage: 
			...
			if read_from_blockchain:
				...
				if addr in MyGlobals.initial_storage:
					value=MyGlobals.initial_storage[addr]
				else:
					value=0
			else:
				value = 0
			...
			MyGlobals.storage_temp[addr] = value
	...
	if op == 'SSTORE':
		...
		MyGlobals.storage_temp[addr] = storage[addr]
		MyGlobals.concrete_write.add(addr)
