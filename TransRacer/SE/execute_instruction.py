...
class EVMCoreHelper:
    ...
    def is_fixed(self, s):
        return s['type'] == 'constant' and is_bv_value(simplify(s['z3']))

    def is_undefined(self, s):
        ...
        for i in range(addr - 31, addr):
            if i in mmemory:
                ...
                if not self.is_undefined(mmemory[i]):
                    ...
                    obytes = addr - i;
                    old_value = mmemory[i]['z3']
                    new_value = (old_value & ((2 ** (8 * obytes) - 1) << (8 * (32 - obytes)))) ^ (
                                value['z3'] >> (8 * obytes))
...
class EVMCore(EVMCoreHelper):
    ...
    def _binary(self, o1, o2, step, op='NONE'):
        ...
        if self.is_fixed(o1):
            ...
            if op in ['MUL', 'AND', 'DIV', 'SDIV'] and 0 == val: return {'type': 'constant', 'step': step,
                                                                         'z3': BitVecVal(0, 256)}
        ...
        if op == 'AND':
            ...
            # if is_bv_value(z1) and z1.as_long() in MyGlobals.m.sha_v2slot:
    ...
    def _ternary(self, o1, o2, o3, step, op='NONE'):
        ...
        if o3['type'] == 'constant' and is_bv_value(simplify(o3['z3'])) and 0 == simplify(o3['z3']).as_long(): return {
            'type': 'constant', 'step': step, 'z3': BitVecVal(0, 256)}
    ...
    def execute(self, code, stack, pos, storage, mmemory, data, sha3_values, calldepth, function_hash, actual_key,
                search_enhance, debug, read_from_blockchain):
        ...
        # for statistics
        if (not search_enhance) and op in ['CALL', 'CODESIZE', 'CODECOPY', 'EXTCODESIZE', 'EXTCODECOPY',
                                           'RETURNDATASIZE', 'RETURNDATACOPY', 'DIFFICULTY', 'GAS', 'CREATE',
                                           'CALLCODE', 'DELEGATECALL', 'STATICCALL', 'LOG0', 'LOG1', 'LOG2', 'LOG3',
                                           'LOG4']:
            ...
            if exact_address >= 0 and exact_offset >= 0:
                ...
                if (exact_offset % 32) == 0:  # for now, can deal only with offsets divisible by 32
                    ...
                    for i in range(exact_offset // 32):
                        if (exact_address + i * 32) not in mmemory or not self.is_fixed(
                                mmemory[exact_address + i * 32]):
                            all_good = False
                            break
                    ...
                    if all_good:
                        ...
                        if exact_offset // 32 == 2:
                            MyGlobals.m.sha_v2slot[int(digest, 16)] = [self.get_value(mmemory[exact_address]),
                                                                       self.get_value(
                                                                           mmemory[exact_address + 32])]
                    else:
                        # for statistics
                        if (not search_enhance):
                            if exact_address == -1:
                        ...
                        if search_enhance and is_bv_value(simplify(mmemory[changed_offset]['z3'])):
                            ...
                            if not 'SHA3' + '-' + str(step) + '-' + function_hash in MyGlobals.sha3vardata:
                                ...
                                if not temp_key in MyGlobals.sha3vardata[
                                    'SHA3' + '-' + str(step) + '-' + function_hash]:
                                    MyGlobals.sha3vardata['SHA3' + '-' + str(step) + '-' + function_hash].append(
                                        temp_key)
        ...
        # otherwise, below, they will get fixed value (BitVecVal) as specified
            ...
            stack.append(
                {'type': 'constant', 'step': step, 'z3': BitVecVal(int(get_params('block_number', ''), 16), 256)})
        elif op == 'GASLIMIT':
            stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(int(get_params('gas_limit', ''), 16), 256)})
        elif op == 'TIMESTAMP':
            stack.append(
                {'type': 'constant', 'step': step, 'z3': BitVecVal(int(get_params('time_stamp', ''), 16), 256)})

        elif op == 'CALLVALUE':
            stack.append(
                {'type': 'constant', 'step': step, 'z3': BitVecVal(int(get_params('call_value', ''), 16), 256)})
        elif op == 'ADDRESS':
            stack.append(
                {'type': 'constant', 'step': step, 'z3': BitVecVal(int(MyGlobals.contract_addr,16),256) })#'z3': BitVecVal(int(get_params('contract_address', ''), 16), 256)
        elif op == 'ORIGIN':
            stack.append(
                {'type': 'constant', 'step': step, 'z3': BitVecVal(int(get_params('contract_address', ''), 16), 256)})
        elif op == 'GASPRICE':
            ...
            stack.append(
                {'type': 'constant', 'step': step, 'z3': BitVecVal(int(get_params('my_address', ''), 16), 256)})
        elif op == 'GAS':
            ...
            stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(0x123, 256)})
        elif op == 'SELFBALANCE':        stack.append({'type': 'constant', 'step': step,
                          'z3': BitVecVal(get_params_bd(actual_key, 'balance', MyGlobals.contract_addr), 256)})
        elif op == 'BASEFEE':
            stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(int('0x123', 16), 256)})
        elif op == 'STATICCALL':
            stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(0xfffffffff, 256)})
        elif op == 'RETURNDATASIZE':
            stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(32, 256)})
        elif op == 'BALANCE':
            v=get_params_bd(actual_key, 'balance', MyGlobals.contract_addr)
            if isinstance(v,int):
                stack.append({'type': 'constant', 'step': step,'z3': BitVecVal(v, 256)})
            else:
                stack.append({'type': 'constant', 'step': step,'z3':v})
        elif op == 'POP':
            pass
        elif op.find('LOG') >= 0:
            pass
        ...
        elif op in ['STOP', 'RETURN', 'REVERT', 'INVALID']:
            ...
            if self.is_fixed(addr):
                ...
                if addr == 0:
                    ...
                    if function_hash in ['11111111', '22222222']:
                        ...
                        stack.append({'type': 'constant', 'step': step,
                                      'z3': BitVecVal(int(function_hash.ljust(64, '0'), 16), 256)})
                # If symmbolic variable does not exist, then create it
                else:

                    if ('data-' + str(key) + '-' + str(addr) + '-' + function_hash) not in data:
                        data['data-' + str(key) + '-' + str(addr) + '-' + function_hash] = BitVec(
                            'input' + str(key) + '[' + str(addr) + ']' + '-' + function_hash, 256)

                    stack.append({'type': 'constant', 'step': step,
                                  'z3': data['data-' + str(key) + '-' + str(addr) + '-' + function_hash]})
                ...
                if debug:
                    print('\033[95m[-] In CALLDATALOAD the input address cannot be determined at step %x: \033[0m' %
                          code[pos]['id'])
                return pos, True
            ...
            MyGlobals.funcs_involve_call.add(function_hash)
            if self.is_fixed(args[5]) and self.is_fixed(args[6]):
                ...
                if (not args[2]['z3'] == None) or (not args[1]['z3'] == None):
                    ...
                    MyGlobals.call_vars.append((step, actual_key, receive_address, ethers))
            ...
            stack.append({'type': 'constant', 'step': step, 'z3': BitVec('call_at_step_' + str(step),
                                                                         256) & 0x1})  # assume the result of call can be any (True or False)
            ...
            for i in range(length // 32):
                data[datapos + 32 * i] = BitVec(
                    'input' + str(key) + '[' + str(datapos + 32 * i) + ']' + '-' + function_hash, 256)
                self.store_in_memory(mmemory, memaddr + 32 * i,
                                     {'type': 'constant', 'step': step, 'z3': data[datapos + 32 * i]})
        ...
        # Truncated the storing only to 32 byte values
            ...
            if self.is_undefined(addr) or not is_bv_value(simplify(addr['z3'])):
                ...
                if (not search_enhance):
                    if 'mstore_mem' in MyGlobals.notimplemented_ins:
                        ...
                        MyGlobals.notimplemented_ins['mstore_mem'] = 1
                if debug: print(
                    '\033[95m[-] The MSTORE the write address on %x  cannot be determined\033[0m' % code[pos]['id'])
                return pos, True
            ...
            if not self.is_fixed(addr):
                ...
                if debug: print(
                    '\033[95m[-] The MSTORE8 the write address on %x  cannot be determined\033[0m' % code[pos]['id'])
                return pos, True
            ...
            if (ea // 32) * 32 not in mmemory:
                mmemory[(ea // 32) * 32] = {'type': 'constant', 'step': step,
                                            'z3': BitVecVal(ev << (31 - (ea % 32)), 256)}
            ...
            if is_bv_value(simplify(addr['z3'])):
                ...
                if not exact_address in MyGlobals.m.rwaddr2id:
                    MyGlobals.m.rwaddr2id[exact_address] = set()
                MyGlobals.m.rwaddr2id[exact_address].add((actual_key,step))
                if exact_address < 100:
                    MyGlobals.m.rs_temp.add(exact_address)
                    MyGlobals.m.rs_temp_id.add(step)
                v = exact_address
                while v in MyGlobals.m.sha_v2slot.keys():
                    slot = MyGlobals.m.sha_v2slot[v][1]
                    if slot < 100:
                        MyGlobals.m.rs_temp.add(slot)
                        MyGlobals.m.rs_temp_id.add(step)
                    v=MyGlobals.m.sha_v2slot[v][1]
                ...
                if is_bv_value(addr):
                    ...
                    if not function_hash in MyGlobals.funcvardata:
                        ...
                        if not hex(exact_address).lstrip('0x').rstrip('L') in MyGlobals.funcvardata[function_hash]:
                            MyGlobals.funcvardata[function_hash][hex(exact_address).lstrip('0x').rstrip('L')] = []
                            MyGlobals.funcvardata[function_hash][hex(exact_address).lstrip('0x').rstrip('L')].append(
                                'R')
            ...
            if is_bv_value(addr):
                ...
                if exact_address in storage:
                    ...
                    if search_enhance and MyGlobals.set_storage_symbolic:
                        stack.append({'type': 'constant', 'step': step,
                                      'z3': BitVec('SLOAD' + '-' + str(step) + '-' + function_hash, 256)})
                        return pos + 1, False
                    ...
                    if exact_address in MyGlobals.storage_temp :
                        ...
                        if exact_address in MyGlobals.addr2val:
                            v = MyGlobals.addr2val[exact_address]
                            t = {'type': 'constant', 'step': step, 'z3': BitVecVal(v, 256)}
                        else:
                            t = {'type': 'constant', 'step': step, 'z3': BitVecVal(0, 256)}
                            MyGlobals.addr2val[exact_address] = 0
                    storage[exact_address] = [t]
                    res = copy.deepcopy(t)

                stack.append(res)
                if not search_enhance:
                    MyGlobals.addr_read.append((actual_key, exact_address, res))
            else:
                if search_enhance:
                    stack.append({'type': 'constant', 'step': step,
                                  'z3': BitVec('SLOAD' + '-' + str(step) + '-' + function_hash, 256)})
            ...
            if is_bv_value(simplify(addr['z3'])):
                ...
                MyGlobals.m.rwaddr2id[exact_address].add((actual_key,step))
                if exact_address < 100:
                    MyGlobals.m.ws_temp.add(exact_address)
                    MyGlobals.m.ws_temp_id.add(step)
                v=exact_address
                while v in MyGlobals.m.sha_v2slot.keys():
                    slot=MyGlobals.m.sha_v2slot[v][1]
                    if slot<100:
                        MyGlobals.m.ws_temp.add(slot)
                        MyGlobals.m.ws_temp_id.add(step)
                    v=MyGlobals.m.sha_v2slot[v][1]
                ...
                if is_bv_value(simplify(addr['z3'])):
                    ...
                    if not function_hash in MyGlobals.funcvardata:
                        ...
                        if not hex(exact_address).lstrip('0x').rstrip('L') in MyGlobals.funcvardata[function_hash]:
                            ...
                            if not 'W' in MyGlobals.funcvardata[function_hash][
                                hex(exact_address).lstrip('0x').rstrip('L')]:
                                MyGlobals.funcvardata[function_hash][
                                    hex(exact_address).lstrip('0x').rstrip('L')].append('W')
            ...
            if is_bv_value(simplify(addr['z3'])):
                ...
                if not search_enhance:
                    key = actual_key
                    MyGlobals.addr_write.append((key, exact_address, t))
                ...
                if debug:
                    print(
                        '\033[95m[-] In SSTORE the write address cannot be determined at step %x: \033[0m' % code[pos][
                            'id'])
                return pos, True
            ...
            if (new_position < 0):
                if debug: print('\033[95m[-] The code has no such JUMP destination: %s at line %x\033[0m' % (
            ...
            stack.append(res)
            if actual_key == 2: MyGlobals.stack2.append(res)
        else:
            executed = False
