...
def get_payable_stateM_from_abi(abi):
    payable = {}
    state_M={}
    hash=None
    for field in abi:
        if field['type'] == 'function':
            ...
            if hash :
                payable[hash] = field['payable']
                if field['stateMutability'] in ['view', 'pure']:
                    state_M[hash]= False
                else:
                    state_M[hash] = True

        elif field['type'] == 'constructor':
            payable['constructor'] = field['payable']
            if field['stateMutability'] in ['view', 'pure']:
                state_M[hash] = False
            else:
                state_M[hash] = True
            ...
            payable['constructor'] = field['payable']
            if field['stateMutability'] in ['view', 'pure','constant']:
                state_M[hash] = False
            else:
                state_M[hash] = True
    if not "fallback" in payable:
        payable['fallback'] = True
        state_M['fallback'] = True
    return payable,state_M
