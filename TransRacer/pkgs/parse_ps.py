...
def get_active_outputs(max_num):
    ...
    for i in range(max_num):
        ps = subprocess.Popen(['fuser', 'out'+str(i)+'.function_num.txt'], stdout=subprocess.PIPE, stderr=devnull)
...
with open('toexec.sh','w') as f:
    ...
    for i in range(no_cores):
        if i not in active_outputs:
            ...
            f.write('nohup python main_v1.py --par  '+ str(i)+' ' + str(no_cores)+' oyente_flagged1.function_num.txt >> out'+str(i)+'.function_num.txt &\n')
            f.write('sleep .5\n')
